...
arg1, arg2, arg3 = map(int, argv[1:])
print(f'выработка часов: {arg1}, ставка в час: {arg2}, премия: {arg3}, заработная плата за месяц: {int(arg1 * arg2 * (arg3/100+1))}')
...
ls_orig = [300, 2, 12, 44, 1, 1, 4, 10, 7, 1, 78, 123, 55]

ls_new = [ls_orig[i + 1] for i in range(len(ls_orig) - 1) if ls_orig[i + 1] > ls_orig[i]]
...
ls_orig2 = [2, 2, 2, 7, 23, 1, 44, 44, 3, 2, 10, 7, 4, 11]

print([el for el in ls_orig2 if ls_orig2.count(el) == 1])
...
print(reduce(multip, [el for el in range(10, 101, 2)])) # уменьшил диапазон в 10 раз, чтобы результат уместился на экране
